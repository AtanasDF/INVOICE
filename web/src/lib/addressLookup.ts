...
export type AddressMatch = {
  ...
  lines?: string[];
  // Only part of an address (a street, or a postcode's town): it goes in
  // under whatever first line is already there instead of replacing it.
  partial?: boolean;
};
...
const LINE_POSTCODE = /\b[A-Z]{1,2}\d[A-Z\d]? ?\d[A-Z]{2}\b/i;
...
// A partial pick keeps the lines already typed above it (the house and
// street) and replaces the old town and postcode: the postcode line, and
// the line above it unless that's the first.
export function mergeAddress(existing: string, match: AddressMatch): string {
  ...
  let kept = existing
    ...
    .map((l) => l.trim())
    .filter(Boolean);
  const at = kept.findIndex((l) => LINE_POSTCODE.test(l));
  if (at >= 0) kept = kept.slice(0, at > 1 ? at - 1 : at);
...
const council = (name: string) => /shire$/i.test(name);
...
export function postcodeTown(postcode: string, builtUpArea: string | null | undefined, neighbours: string[], district: string | null | undefined): string {
  ...
  return common ?? (district ?? "").replace(/^City of /i, "").trim();
}
...
function osmTown(p: OsmProperties): string {
  ...
  if (city && !(council(city) && p.district)) return city;
  return (p.district || p.locality || city || p.county || "").trim();
}

export function osmMatch(p: OsmProperties, id: string, townOverride?: string): AddressMatch | null {
  ...
  const postcodeHere = p.postcode ? normalisePostcode(p.postcode) : null;
  const town = townOverride || (postcodeHere && LONDON.test(postcodeHere) ? "London" : osmTown(p));
  const postcode = postcodeHere;
  if (isStreet) {
    const name = (p.name || street || "").trim();
    if (!name || !town) return null;
    const lines = [name, town, postcode ?? ""].filter(Boolean);
    return { id, label: name, detail: [town, postcode].filter(Boolean).join(" "), lines, partial: true };
  }
