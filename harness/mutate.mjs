...
//
// NEVER COMMIT A MUTATED TREE. `revert` is a git checkout of web/src, so
// commit real work before applying anything.
...
if (cmd === "revert") {
  execSync("git checkout -- web/src", { cwd: "/Users/nasko/Desktop/INVOICE" });
  const dirty = execSync("git status --porcelain web/src", { cwd: "/Users/nasko/Desktop/INVOICE" }).toString().trim();
...
if (cmd === "apply") console.log(`\n${applied} mutation(s) applied. Rebuild, run the harness, then: node mutate.mjs revert`);
