import { launch, sleep, shot, clickText, zoom, hint, openScanner } from "./camera2.mjs";
const results = [];
...
await run("small-then-gone.mjpeg", async (page) => {
  ...
  check("page still found after zoom", !/Fit the page/.test((await hint(page)) ?? ""), await hint(page));
...
await run("large.mjpeg", async (page) => {
  ...
  check("large page found", !/Fit the page/.test((await hint(page)) ?? ""), await hint(page));
});
await run("side.mjpeg", async (page) => {
  ...
  const z = await zoom(page);
  check("off-centre page zooms only as far as keeps it in view", !/Fit the page/.test((await hint(page)) ?? "") && !isOne(z) && z !== "scale(2)", `${z} / ${await hint(page)}`);
});
