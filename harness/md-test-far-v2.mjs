...
import { launch, openScanner, sleep, zoom, hint, shot, measure } from "./md-camera3.mjs";
const results = [];
...
if (want("dark")) await run("dark-thing.mjpeg", {}, async (page) => {
  await openScanner(page);
  const { hints, zooms } = await watch(page, 6000);
  check("dark phone-like object: not taken for a page", hints.every((h) => !h) && Math.max(...zooms) === 1, `${[...new Set(hints)].join(" | ")} max ${Math.max(...zooms)}`);
});
