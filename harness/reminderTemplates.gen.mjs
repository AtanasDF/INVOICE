...
export const DEFAULT_REMINDER_TEXT = {
    ...
    after: "Hi {{client_name}}, invoice {{invoice_number}} was due on {{due_date}} and £{{amount_due}} of it is now overdue. Please arrange payment within the next 7 days, or reply to this email if there's a problem with it.",
    late: "Hi {{client_name}}, invoice {{invoice_number}} is now two weeks overdue (it was due on {{due_date}}), with £{{amount_due}} still to pay. Please pay it by {{pay_by}}. If there's a reason it can't be paid, reply to this email so we can sort it out.",
    final: "Hi {{client_name}}, this is a final reminder: invoice {{invoice_number}} was due on {{due_date}} and is 30 days overdue, with £{{amount_due}} still to pay. Please pay it by {{pay_by}}. If you think something is wrong with the invoice, reply to this email straight away.",
};
...
export function reminderDueToday(dueDate, today) {
...
export function laterReminders(kind) {
...
export function lateCompensation(amount) {
...
export function reminderBody({ kind, template, clientName, clientIsCompany, invoiceNumber, amountDue, dueDate, today, bank, businessName, claimInterest }) {
    ...
    const interest = kind === "final" && claimInterest && clientIsCompany
