import fs from "fs";
import { makeDb, launchSignedIn, signIn, sleep, clickText, bodyText, shot, newId } from "./qr-regress-mockdb.mjs";
const BASE = "http://localhost:3305";
const DL = fs.mkdtempSync("/private/tmp/claude-501/-Users-nasko-Desktop-MM-INVOICES-AUTO/85090693-d203-4a4e-be6a-5911529c13ff/scratchpad/qr/dl-") + "/";
...
try {
  ...
  const options = await page.evaluate(() => [...document.querySelectorAll('[role="radiogroup"][aria-labelledby] > div')].map((g) => [g.querySelector("p").textContent, ...[...g.querySelectorAll('[role="radio"]')].map((b) => b.querySelector("span span").textContent)]));
  check("live clients then suppliers offered (no archived)", JSON.stringify(options) === JSON.stringify([["Clients", "Jane Customer"], ["Suppliers", "Supplier Co"]]), JSON.stringify(options));
  ...
  await shot(page, "qux-quotes-new");
  ...
  await shot(page, "qux-quote-draft");
  ...
  await shot(page, "qux-quotes-list");
  ...
  await shot(page, "qux-quotes-error");
} finally {
  await browser.close();
