...
for f in pathlib.Path("gen").rglob("*.js"):
    ...
    # A lib file that imports a sibling relatively compiles to an
    # extensionless specifier, which Node refuses outright. Loud, but
    # confusing the first time.
    t2 = re.sub(r'from "(\./[\w-]+)"(?!\s*\.js)', r'from "\1.js"', t2)
...
SUITES=(
  ...
  # In DEV_SERVER below, so it runs -- but it was missing from this list, and
...
DEV_SERVER=(test-check-company test-inbox-ingest test-feedback test-send-document test-public-logo test-address-stubbed test-public-request test-links test-quote-links test-quote-requests test-vat-lookup test-locked-doors test-help-chat-live)
