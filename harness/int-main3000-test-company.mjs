...
try {
  ...
  await sleep(500);
  const nameSel = 'input[role="combobox"]:not([placeholder^="Find address"])';
  ...
  await page.evaluate(() => document.querySelectorAll('input[role="combobox"]:not([placeholder^="Find address"])')[0].setAttribute("data-t", "issuer"));
  await page.evaluate(() => document.querySelectorAll('input[role="combobox"]:not([placeholder^="Find address"])')[1].setAttribute("data-t", "customer"));
  ...
  const issuer = await page.evaluate(() => {
    ...
    return { name: document.querySelector('[data-t="issuer"]').value, address: [...document.querySelectorAll("span")].find((x) => x.textContent.trim() === "Address")?.closest("div")?.querySelector("textarea")?.value, company: byLabel("Company number")?.value };
  });
  ...
  await sleep(300);
  const client = await page.evaluate(() => ({ name: document.querySelector('input[role="combobox"]:not([placeholder^="Find address"])').value, address: document.querySelector('textarea[placeholder^="Billing address"]').value }));
  ...
  await page.evaluate(() => document.querySelector('[role="option"]').click());
  await sleep(300);
  const settings = await page.evaluate(() => ({ name: document.querySelector('input[role="combobox"]:not([placeholder^="Find address"])').value, address: [...document.querySelectorAll("textarea")][0].value, offer: document.body.innerText.includes("Registered office: 12 High Street, Leeds, LS1 2AB") }));
  ...
  await sleep(500);
  check("lookup off: plain inputs, no combobox role", (await p2.$$('input[role="combobox"]:not([placeholder^="Find address"])')).length === 0);
