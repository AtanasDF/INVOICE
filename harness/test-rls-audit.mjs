...
const noRls = [];
const noPolicy = [];
// migration-031 switches several on at once through a loop, so its list
// counts as much as a literal ALTER does. This reads the SQL files, not
// the live database -- the query at the foot of migration-031 is what
// proves the database itself.
const lockedDown = (readFileSync(`${DIR}/migration-031-lock-down-early-backups.sql`, "utf8").match(/'(\w+_backup_\w+)'/g) ?? []).map((x) => x.replaceAll("'", ""));
for (const [t, where] of tables) {
  if (!new RegExp(`alter table (?:public\\.)?${t}\\s+enable row level security`, "i").test(all) && !lockedDown.includes(t)) noRls.push(`${t} (${where})`);
