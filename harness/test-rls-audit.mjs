...
const noRls = [];
const noPolicy = [];
// migration-031 enables RLS on every %_backup_% table through a loop, so
// a backup table counts as covered when the loop is there. This reads the
// SQL files, not the live database: on 2026-09-21 it reported six backup
// tables open that the database had locked all along (000/001 never
// wrote the ALTER, but RLS was on). What proves the database is the query
// at the foot of migration-031, run against it.
...
for (const [t, where] of tables) {
  if (!new RegExp(`alter table (?:public\\.)?${t}\\s+enable row level security`, "i").test(all) && !(lockedDown && /_backup_/.test(t))) noRls.push(`${t} (${where})`);
