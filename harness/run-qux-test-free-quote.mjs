...
const DL = new URL("./downloads-qux/", import.meta.url).pathname;
...
try {
  ...
  const pre = await page.evaluate(() => ({ name: document.querySelector('input[placeholder="e.g. Acme Kitchens Ltd"]')?.value, company: document.querySelector('button[role="radio"][aria-checked="true"]')?.textContent }));
  check("imported customer opens as a new customer, prefilled, Company picked", pre.name === "Acme Kitchens Ltd" && pre.company === "Company", JSON.stringify(pre));
  await clickText(page, "Save quote");
  await sleep(300);
  check("can't save the quote over a half-added customer", (await bodyText(page)).includes("Finish adding the new customer") && !db.tables.quotes?.length);
  await clickText(page, "Add customer");
  await page.waitForFunction(() => !document.body.innerText.includes("Saved to your clients, so it"), { timeout: 10000 });
