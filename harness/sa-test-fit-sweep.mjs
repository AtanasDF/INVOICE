...
const inv = (o) => ({ id: newId(), user_id: "x", client_id: C.id, date: today, number: "INV-000123", items: [{ description: "Skim coat to kitchen ceiling and walls including making good", quantity: 3, unitPrice: 1234.56, vatRate: "standard" }], notes: "Thanks", due_date: today, payment_terms: "30 days", status: "sent", tags: ["Site A"], vat_registered: true, cis_rate: null, ...o });
...
db.tables.invoice_payments.push({ id: newId(), user_id: "x", invoice_id: PAID.id, date: today, amount: 4444.42, method: "bank", note: "" });
const Q = { id: newId(), user_id: "x", client_id: C.id, number: "Q-0100", date: today, valid_until: today, items: [{ description: "Skim coat", quantity: 1, unitPrice: 500, vatRate: "standard" }], notes: "", status: "sent", invoice_id: null, deposit_percent: 25, deposit_amount: null, deposit_invoice_id: null, deposit_claimed: false };
db.tables.quotes.push(Q);
db.tables.receipts.push({ id: newId(), user_id: "x", client_id: S.id, date: today, vendor: "Builders Merchant Supplies (Bristol) Limited", category: "Materials", amount: 100, vat_amount: 20, image_data_url: null, notes: "", starred: false, needs_review: false, warranty_months: null, tags: [], line_items: [], document_type: "invoice", invoice_number: "IN178077", due_date: today, paid: false, details: { orderNumber: "ORD-123456789" }, credit_of_receipt_id: null, original_amount: null, original_vat_amount: null, original_currency: null, fx_rate: null });

const WIDTH = Number(process.env.WIDTH ?? 375);
const { browser, page } = await launchSignedIn(db, { base: BASE, width: WIDTH, profile: "profile-sa-fit-sweep" });
...
try {
  ...
  for (const p of pages) try {
    ...
    await sleep(700);
    const wide = await page.evaluate(() => [...document.querySelectorAll("body *")].filter((e) => { const r = e.getBoundingClientRect(); return r.width > 0 && r.right > window.innerWidth + 1 && getComputedStyle(e).position !== "fixed"; }).filter((e, _, all) => !all.some((o) => o !== e && o.contains(e))).slice(0, 3).map((e) => `${e.tagName}.${String(e.className).slice(0, 50)} r=${Math.round(e.getBoundingClientRect().right)} "${(e.innerText ?? "").replace(/\s+/g, " ").slice(0, 40)}"`));
