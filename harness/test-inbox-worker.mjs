...
const pdfAttachment = (name) => part([`Content-Type: application/pdf; name="${name}"`, `Content-Disposition: attachment; filename="${name}"`, "Content-Transfer-Encoding: base64"], PDF);
...
function email({ to = `u-${TOKEN}@invoiceover.com`, icons = 0, attachments = [], html = "<p>Please find the invoice attached.</p>", plain = true } = {}) {
  const related = [
  ...
  const body = plain
    ? ['Content-Type: multipart/alternative; boundary="alt"', "", "--alt", part(["Content-Type: text/plain; charset=utf-8"], html.replace(/<[^>]+>/g, "")), "--alt", related, "--alt--"].join("\r\n")
    : related;
  return [
